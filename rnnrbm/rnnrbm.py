...
class RnnRbm:
    ...
    def generate(self, filename, show=True):
        ...
        piano_roll_sm = np.reshape(piano_roll, [-1, 88, 2])

        noteStateMatrixToMidi(piano_roll_sm, name=filename)

        if show:
            extent = (0, self.dt * len(piano_roll_sm)) + self.r
            pylab.figure()
...
def biaxial_encoding_read(filename):
    sm = midiToNoteStateMatrix(filename)
 
    # input_form = noteStateMatrixToInputForm(sm)

    sm_flat = np.reshape(sm, [-1, 88*2])
    return sm_flat


def test_rnnrbm(model, dataset, batch_size=64, num_epochs=1):
    model.train(dataset,
...
if __name__ == '__main__':
    ...
    re = os.path.join(os.path.split(os.path.dirname(__file__))[0],
			...
			#'..', 'biaxial-rnn-music-composition', 'music', '*.mid')
    ...
    dataset = [biaxial_encoding_read(f).astype(theano.config.floatX)
               for f in files]

    model = RnnRbm()

    for i in range(NUM_EPOCHS):
        model = test_rnnrbm(model, dataset, num_epochs=1)

        if i % 10 == 0:
            model.generate('rnnrbm_sm_{}_1.mid'.format(i))
            model.generate('rnnrbm_sm_{}_2.mid'.format(i))
